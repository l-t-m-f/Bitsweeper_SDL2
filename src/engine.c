...
SDL_Surface *surface_array_object[] = {};
...
engine_init (void)
{
  ...
  main_window = SDL_CreateWindow ("Bitsweeper",
...
void engine_render_loop (void)
{
  ...
  SDL_SetRenderDrawColor (main_renderer, main_palette->colors[11].r, main_palette->colors[11].g, main_palette->colors[11].b, main_palette->colors[11].a);
...
int8_t
engine_handle_error (MSErrorType error_type)
{
  int8_t result = 0;
  switch (error_type)
    {
      ...
      case MS_ERROR_TYPE_MAX:
        {
          SDL_Log ("Error - invalid 'MS_ERROR_TYPE_MAX' case reached in switch of function handle_error ().\n");
          result = -1;
          break;
        }
      default:
...
void
engine_load_images (void)
{
  surface_array_object[0] = IMG_Load ("gfx/Tiles.png");
...
SDL_Surface *engine_extract_tile (uint32_t atlas_key, SDL_Point tile_coordinates)
{
  ...
  SDL_BlitSurface (surface_array_object[atlas_key], &sourceTarget, tile_surface, NULL);
  return tile_surface;
}

SDL_Surface *
engine_create_portable_surface (uint8_t width, uint8_t height)
{
...
SDL_Texture *
engine_make_text_texture (char *text, SDL_Color *color)
{
  SDL_Surface *surface;
...
static bool
handle_sys_events (SDL_Event event)
{
  switch (event.type)
    {
...
static SDL_Texture *
surface_to_texture (SDL_Surface *surface, int destroy_surface)
{
  SDL_Texture *texture;
...
/*
 * Create the game's color palette. The palette entries are initialized to white and
 * must be properly set.
 */
static void
create_main_palette (void)
{
  ...
  if (SDL_SetPaletteColors (main_palette,
                            (SDL_Color[]) {
                                ...
                                (SDL_Color) {.r = 0xFF, .g = 0x00, .b = 0xFF, .a = 0xFF},    // MAGENTA
                                ...
                                (SDL_Color) {.r = 0x80, .g = 0x80, .b = 0x80, .a = 0x80},    // LIGHT_GRAY
                                (SDL_Color) {.r = 0x40, .g = 0x40, .b = 0x40, .a = 0xFF},    // DARK_GRAY
                            },
                            0, COLOR_MAX)
      < 0)
    {
      SDL_Log ("Error setting palette colors. Not all colors could be set! (%s)", SDL_GetError ());
    }
